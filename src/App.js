...
// تخزين البيانات عبر Netlify Blobs (دالة Netlify)
...
function saveDB(db) {
  try {
    ...
    fbWrite(db); // حفظ في قاعدة بيانات Netlify
  } catch(e) {}
}
function useNetlifySync(setDB) {
  useEffect(() => {
    // جلب البيانات من قاعدة بيانات Netlify عند التحميل
    fbRead().then(data => {
...
export default function App(){
  ...
  // مزامنة قاعدة بيانات Netlify في الوقت الفعلي
  useNetlifySync(setDB);
...
function LeavesPage({db,persist,user,isAdmin,cy,cm}){
  ...
  const openAdd=()=>{
    setForm({employeeId:isAdmin?db.employees[0]?.id:user.employeeId,date:new Date().toISOString().split("T")[0],type:"annual",reason:"",attachment:null,attachmentName:""});
    setModal(true);
  };
...
المستشفى: ${result.hospital||"—"}`);
  ...
  return(<div>
    ...
    <div style={{background:"#fff",borderRadius:12,overflow:"hidden",border:"1.5px solid #e0e8f4"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        <thead><tr style={{background:"linear-gradient(135deg,#0a2d5e,#1a4a8a)",color:"#fff"}}>
          {["الموظفة","التاريخ","النوع","السبب","مرفق",...(isAdmin?["حذف"]:[])].map(h=>(
            <th key={h} style={{padding:"12px 14px",textAlign:"right"}}>{h}</th>
          ))}
        </tr></thead>
        <tbody>{list.map((l,i)=>{
